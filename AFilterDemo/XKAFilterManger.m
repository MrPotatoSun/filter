...
@interface XKAFilterManger ()

@end



@implementation XKAFilterManger
/*获取 image RGBA 通道的上下文*/
static CGContextRef CreateRGBABitmapContex(CGImageRef inImage){
    
    CGContextRef context = NULL; // 上下文 初始化
    /*        颜色通道         */
...
- (UIImage *)createImageWithImagePi:(UIImage *)inImage andColorMatrix:(const float *)f{
    ...
    if (!provider) {
        ...
        if (!imageRef) {
            ...
            if (_aFilterImageBlock) {
                _aFilterImageBlock(my_image);
            }
